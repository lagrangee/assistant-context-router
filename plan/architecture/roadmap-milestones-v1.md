...
## Milestone 4 — Protocol Routing & Work Visibility
目标：
- 服务 `openclaw-feishu-orchestrator`
- 支持 protocol/project/workflow 分层
- 建立 work orchestration subsystem 的接入边界
- 形成 human-friendly 的最小 work visibility
- 强化 safe-fail 与 trace

通过信号：
- protocol owner / business target 不混用
- unresolved target 不误执行
- task/backlog 与 run/progress/event 不混用
- 长时工作进展可见，但不会污染主会话与 backlog 面
...
## Candidate Note — Agent Autonomy
`agent autonomy` 当前不进入 Step 2 正式范围，先作为后续候选方向保留。

可能包括：
- agent 主动排序 backlog
- agent 主动创建候选任务
- token / budget aware 的主动运营
- secretary 主动引导项目启动或 scope 调整

前置条件：
- Step 4 的 routing / orchestration / visibility 边界已稳定
- task vs run vs signal 的对象模型已收敛
- human override / escalation / notification policy 已明确
